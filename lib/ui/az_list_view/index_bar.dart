...
class _SuspensionListViewIndexBarState extends State<IndexBar> {
  ...
  Widget build(BuildContext context) {
    double tmp =
        winHeight(context) - widget.data.length * widget.itemHeight - 70;
    if (tmp > 0) tmp /= 2;
    return Container(
      alignment: Alignment.center,
      color: _isTouchDown ? widget.touchDownColor : widget.color,
      width: widget.width.toDouble(),
      padding: EdgeInsets.only(top: tmp > 0 ? tmp : 0.0),
      child: _IndexBar(
        data: widget.data,
        width: widget.width,
...
class _IndexBarState extends State<_IndexBar> {
  ...
  Widget build(BuildContext context) {
    ...
    widget.data.forEach((v) {
      bool isCur =
          RegExp("[A-Z]").hasMatch(widget.curTag) && v == widget.curTag;
        ...
        child: (isCur
            ? Container(
                ...
                decoration: BoxDecoration(
                    color: Color(0xFF09C497), shape: BoxShape.circle),
                child: Text(v,
