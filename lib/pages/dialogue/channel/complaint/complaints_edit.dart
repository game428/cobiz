...
import 'package:cobiz_client/http/user.dart' as complaints;
...
class ComplaintsEditPage extends StatefulWidget {
  final int type;
  final int from; //来源 1.用户 2.群聊
  final int targetId;
  const ComplaintsEditPage(
      {Key key, this.type, @required this.from, @required this.targetId})
      : super(key: key);

  @override
  _ComplaintsEditPageState createState() => _ComplaintsEditPageState();
}

class _ComplaintsEditPageState extends State<ComplaintsEditPage> {
  List pathList = List();
  ...
  _sub() async {
    ...
    if (_textController.text.isEmpty)
      return showToast(context, S.of(context).complaintsInfo);
    ...
    if (pathList.length > 0) {
      ...
      var res = await complaints.subComplain(widget.type, widget.from,
          widget.targetId, _textController.text, jsonEncode(strPath));
      Loading.complete();
      if (res != false) {
        showToast(context, S.of(context).complaintSuccessful);
        Navigator.pop(context);
      } else {
        showToast(context, S.of(context).tryAgainLater);
      }
    } else {
      var res = await complaints.subComplain(
          0, 1, 1, _textController.text, jsonEncode(strPath));
      Loading.complete();
      if (res != false) {
        showToast(context, S.of(context).complaintSuccessful);
        Navigator.pop(context);
  ...
  Widget _buildContent() {
    // double height = ScreenData.height - ((pathList.length ~/ 4)) / 4 * ScreenData.width - 140;
    // if (height < 200.0) height = 200.0;
    return Container(
      height: 200.0,
      child: ExtendedTextField(
        ...
        decoration: InputDecoration(
          ...
          hintText: '${S.of(context).complaintsInfo}...',
          isDense: true,
